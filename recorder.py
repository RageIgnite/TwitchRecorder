...
def stop_recording(process_info):
    ...
    if process.poll() is not None:
        logger.debug(f"FFmpeg process for {username} (Output: {output_path}) already terminated with code {process.returncode}.") # Changed to DEBUG
        return

    # 1. Try sending 'q'
    logger.debug(f"Attempting graceful shutdown for {username} (Output: {output_path}) by sending 'q' to FFmpeg stdin...") # Changed to DEBUG
    try:
    ...
    logger.debug(f"Attempting to stop FFmpeg for {username} (Output: {output_path}) with {signal_to_try_name}...") # Changed to DEBUG
    try:
        if os.name == 'nt':
            process.send_signal(signal.CTRL_BREAK_EVENT) 
            logger.debug(f"Sent CTRL_BREAK_EVENT to FFmpeg process group for {username}.") # Changed to DEBUG for consistency
        else:
            os.kill(process.pid, signal.SIGINT)
            logger.debug(f"Sent SIGINT to FFmpeg PID {process.pid} for {username}.") # Changed to DEBUG for consistency
    ...
    logger.debug(f"Attempting to stop FFmpeg for {username} (Output: {output_path}) with SIGTERM...") # Changed to DEBUG
    try:
        process.terminate() 
    ...
    logger.debug(f"Attempting to kill FFmpeg for {username} (Output: {output_path}) with SIGKILL...") # Changed to DEBUG
    try:
        process.kill() 
